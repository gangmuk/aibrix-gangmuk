...
class LLMRoutingDataProcessor:
    ...
    def extract_pod_columns(self, df):
        """Extract pod-related columns and organize by pod ID and feature type."""
        pod_data = defaultdict(dict)
        
        # Get pod IDs from selected_pod column
        if 'selected_pod' in df.columns:
            self.pod_ids = df['selected_pod'].dropna().unique().tolist()
            self.pod_ids = sorted(self.pod_ids)
            logger.info(f"Found pod IDs from selected_pod column: {self.pod_ids}")
        else:
            logger.error(f"No selected_pod column found in DataFrame")
            assert False
        ...
        for col in pod_feature_columns:
            ...
            if pod_id in self.pod_ids:
                ...
                logger.error(f"Pod ID {pod_id} not found in self.pod_ids {self.pod_ids}")
                assert False
    ...
    def preprocess_data(self, df):
        ...
        pod_data = self.extract_pod_columns(df)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Process LLM routing data for RL training')
    ...
    parser.add_argument('--create-loaders', action='store_true',
                        help='Create PyTorch DataLoader objects')
    
    # Parse command line arguments
    if len(sys.argv) == 1:
        args = parser.parse_args(['input.csv'])  # Default for testing
    else:
    ...
    n_samples = len(df)
    test_size = int(n_samples * args.test_split)
    ...
    # Process data
    processor = LLMRoutingDataProcessor(output_dir=args.output_dir)
    
    # Process training data
    logger.info("Processing training data...")
    train_processed = processor.preprocess_data(train_df)
    train_path = processor.save_processed_data(train_processed, prefix="train")
    
    # Process test data
    logger.info("Processing test data...")
    test_processed = processor.preprocess_data(test_df)
    ...
    if args.create_loaders:
        ...
        train_loader, val_loader = processor.create_dataset_loaders(
            train_processed, batch_size=args.batch_size
        )
