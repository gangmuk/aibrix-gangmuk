...
def parse_log_file(file_path):
    ...
    if len(df) == 0:
        ...
        assert False
    logger.info(f"Successfully parsed {len(df)} rows from the log file.")
...
def calculate_slo_satisfaction_for_avg_ttft(row, ttft_slo_threshold):
    try:
        ttft = float(row['ttft'])
        return ttft <= ttft_slo_threshold
    except (ValueError, TypeError):
        return False
...
def calculate_tpot_reward(row, avg_tpot_slo_threshold):
    try:
        avg_tpot = float(row['avg_tpot'])
...
def extract_key_pod_metrics(pod_metrics, pod_id):
    ...
    if pod_id not in pod_metrics:
        ...
        # }
    # logger.info(f"pod_metrics[{pod_id}]: {pod_metrics[pod_id].keys()}")
    return {
...
def get_all_pod_ip(df):
    ...
    for _, row in df.iterrows():
        ...
        logger.info(f"kv_cache_hit_ratios: {kv_cache_hit_ratios}")
        ...
        if len(all_pods_set) == 8:
            ...
            break
    all_pod_ip_list = list(all_pods_set)
    logger.info(f"Identified {len(all_pod_ip_list)} pods: {all_pod_ip_list}")
    return all_pod_ip_list

def preprocess_dataset(df, output_file=None):
    all_pods = get_all_pod_ip(df)
    ...
    if 'podMetricsLastSecond' in df.columns and len(df) > 0:
        ...
        if pod_metrics and len(pod_metrics) > 0:
            logger.info(f"features in pod_metrics: {pod_metrics[list(pod_metrics.keys())[0]].keys()}")
        else:
            logger.error("Error: No pod metrics found in the first row.")
        ...
        if pod_metrics:
            ...
            for pod_id, metrics in pod_metrics.items():
                ...
                logger.debug(f"pod {pod_id}, pod_metrics: {metrics}")
    ...
    processed_df['avg_ttft_slo_satisfied'] = processed_df['ttft'].apply(
        lambda x: x <= ttft_slo_threshold)

    processed_df['ttft_reward'] = processed_df.apply(
        lambda row: calculate_ttft_reward(row, ttft_slo_threshold), axis=1)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(input_file):
        logger.error("Input file does not exist. exiting...")
        exit()
    ...
    try:
        processed_df, mapping_info, all_pods = preprocess_dataset(df, output_file)
    ...
    logger.info(f"* avg_tpot_slo_threshold: {avg_tpot_slo_threshold} ms")
    logger.info(f"* ttft_slo_threshold: {ttft_slo_threshold} ms")
